...
type Tab = 'logos' | 'colors' | 'typography' | 'assets' | 'grid';
...
export default function BrandkitPage() {
    ...
    const handleGridUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        const effectiveClientId = clientId || selected?.client;
        if (!file || !effectiveClientId) {
            setActionError('Selecione um Brand Kit ou Cliente primeiro.');
            return;
        }
        withLoading(async () => {
            console.log('Target effectiveClientId:', effectiveClientId);
            await createGrid({ client: effectiveClientId, file });
            const updated = await fetchGrids(effectiveClientId);
            setGrids(updated);
        });
        e.target.value = '';
    };

    const handleGridUrlAdd = async () => {
        const effectiveClientId = clientId || selected?.client;
        if (!gridUrl.trim() || !effectiveClientId) {
            setActionError('Cole uma URL e selecione um Brand Kit primeiro.');
            return;
        }
        withLoading(async () => {
            console.log('Target effectiveClientId (URL):', effectiveClientId);
            await createGrid({ client: effectiveClientId, url: gridUrl });
            const updated = await fetchGrids(effectiveClientId);
            setGrids(updated);
            setGridUrl('');
        });
    ...
    const handleGridDelete = async (id: number) => {
        if (!confirm('Remover esta referência de grid?')) return;
        const effectiveClientId = clientId || selected?.client;
        withLoading(async () => {
            await deleteGrid(id);
        ...
        if (activeTab === 'grid') {
            return (
                <div className="space-y-6">
                    {isAdmin && (
                        <div className="p-6 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-6">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <h4 className="text-base font-bold text-gray-800 dark:text-white flex items-center gap-2">
                                    ...
                                    Gerenciar Referências de Grid
                                </h4>
                                {!clientId && (
                                    <div className="w-full md:w-72">
                                        <Select
                                            options={clients.map(c => ({ value: String(c.id), label: c.name }))}
                                            placeholder="Selecionar Cliente para Grid..."
                                            onChange={(val) => setClientId(parseInt(val))}
                                            defaultValue={clientId ? String(clientId) : ''}
                                        />
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                                <div className="flex-1">
                                    <input
                                        ...
                                        placeholder="Cole um link de imagem (Instagram Grid URL)..."
                                        className="w-full text-sm px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/20"
                                    />
                                </div>
                                <div className="flex gap-2">
                                    <button
                                        onClick={handleGridUrlAdd}
                                        disabled={actionLoading || !gridUrl.trim() || !(clientId || selected?.client)}
                                        className="px-6 py-3 rounded-xl bg-brand-500 text-white text-sm font-bold hover:bg-brand-600 disabled:opacity-50 transition-all shadow-md shadow-brand-500/10 whitespace-nowrap"
                                    >
                                        Adicionar URL
                                    </button>
                                    <input ref={gridInputRef} type="file" accept="image/*" className="hidden" onChange={handleGridUpload} />
                                    <button
                                        onClick={() => gridInputRef.current?.click()}
                                        disabled={actionLoading || !(clientId || selected?.client)}
                                        className="px-6 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 text-sm font-bold hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 transition-all whitespace-nowrap flex items-center gap-2"
                                    >
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            {!(clientId || selected?.client) && (
                                <p className="text-xs text-amber-500 flex items-center gap-1">
                                    ...
                                    Selecione um cliente para habilitar o upload.
                                </p>
                            )}
                        </div>
                    )}
                    ...
                    {loadingGrids ? (
                        <p className="text-sm text-gray-500">Carregando grids...</p>
                    ) : grids.length === 0 ? (
                        <p className="text-sm text-gray-400 py-10 text-center">Nenhuma referência de grid adicionada.</p>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {grids.map(grid => (
                                <div key={grid.id} className="group relative rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden bg-white dark:bg-gray-900 shadow-sm hover:shadow-md transition-all">
                                    <div className="aspect-[4/5] flex items-center justify-center bg-gray-50 dark:bg-gray-800">
                                        <img
                                            src={grid.image_url}
                                            alt="Social Grid Reference"
                                    ...
                                    <div className="absolute top-2 right-2 flex gap-1 z-10">
                                        <a
                                            ...
                                            className="p-2 rounded-full bg-white/90 dark:bg-gray-900/90 text-gray-700 dark:text-gray-300 hover:text-brand-500 shadow-md backdrop-blur-sm transition-all"
                                            title="Abrir imagem"
                                        >
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        {isAdmin && (
                                            <button
                                                onClick={() => handleGridDelete(grid.id)}
                                                className="p-2 rounded-full bg-red-500 text-white hover:bg-red-600 shadow-lg transition-all active:scale-90"
                                    ...
                                    <div className="p-3 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
                                        <p className="text-[10px] text-gray-400 font-mono italic">
                                            ID: {grid.id} • {new Date(grid.created_at).toLocaleDateString()}
                                        </p>
                                    </div>
                                </div>
                            ))}
                ...
                return (
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            <input ref={logoInputRef} type="file" accept="image/*,video/*,application/pdf" className="hidden" onChange={handleLogoUpload} />
                            <button
    ...
    const tabs: { key: Tab; label: string; count: number }[] = [
        ...
        { key: 'grid', label: 'Social Grid', count: grids.length },
    ];
            ...
            <div className="space-y-6">
                ...
                {showCreate && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <div className="relative bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col">
                            <div className="p-6 overflow-y-auto">
                                <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-6">Criar Brand Kit</h3>
                                <div className="space-y-4">
                                    <input
                                        type="text" value={createName} onChange={e => setCreateName(e.target.value)}
                                        placeholder="Nome do Brand Kit *"
                                        className="w-full text-sm px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/20"
                                    />
                                    <textarea
                                        value={createDesc} onChange={e => setCreateDesc(e.target.value)}
                                        placeholder="Descrição (opcional)"
                                        rows={3}
                                        className="w-full text-sm px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 resize-none"
                                    />
                                    {isAdmin && (
                                        <div>
                                            <label className="text-xs font-semibold text-gray-400 mb-2 block uppercase tracking-wider">Cliente Responsável *</label>
                                            <Select
                                                options={clients.map(c => ({ value: String(c.id), label: c.name }))}
                                    ...
                                    {createError && (
                                        <div className="p-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-xs text-red-500 border border-red-100 dark:border-red-900/30">
                                            {createError}
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="p-6 bg-gray-50 dark:bg-gray-800/50 flex gap-3">
                                <button onClick={() => { setShowCreate(false); setCreateError(''); }} className="flex-1 py-3 rounded-2xl border border-gray-200 dark:border-gray-700 text-sm font-semibold text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                                    Cancelar
                                </button>
                                <button onClick={handleCreate} disabled={creating} className="flex-1 py-3 rounded-2xl bg-brand-500 text-white text-sm font-bold hover:bg-brand-600 disabled:opacity-50 transition-all shadow-lg shadow-brand-500/20 active:scale-[0.98]">
